...
import { SubTitulo } from '../../componentes/ui/Sub-Titulo';
...
export const CustomSectionListScreen = () => {
  ...
  return (
    <VistaPersonalizada margin>
      <Titulo texto='Lista de personajes' zonaSegura />

      <Tarjeta>
        <SectionList 
          sections={ houses }
          keyExtractor={ (item) => item }
          renderItem={ ({ item }) => <Text style={{ color: colors.texto, marginVertical: 2 }}>{ item }</Text> }

          renderSectionHeader={ ({section})=> <SubTitulo texto={ section.title } colorFondo={ colors.TarjetaFondo } /> }
          stickySectionHeadersEnabled

          SectionSeparatorComponent={ Separador }
          ListHeaderComponent={ () => <Titulo texto="Personajes" /> }
          ListFooterComponent={ () => <Titulo texto={ `Secciones: ${ houses.length }` } /> }
